    ...
    return (
        <div className='flex md:space-x-4 md:flex-row flex-col'> 
         <div className='flex gap-3 md:flex-row  flex-col w-full md:w-1/2'>
            ...
            <PrimaryButton onClick={(e)=>{e.preventDefault();propertyName.current.value && (!properties.some(prop => prop.name === propertyName.current.value)) && addProperty({name:propertyName.current.value,type:propertyType.current.value}); propertyName.current.value = ''; propertyType.current.value = '';}} className='w-full h-12 hover:cursor-pointer flex justify-center' disabled={isDisabled}>
                Add Item
            </PrimaryButton>
          </div>  
